...
import {findSectionHeader} from "./../../src/req/icdToken";
...
it(`should find token in section`,() => {
    let res = findSectionHeader(zeroOne,gastro.completionItem.label);
    expect(res!.completionItem.label).toBe(gastro.completionItem.label);
});

it(`should find token in section`,() => {
    let res = findSectionHeader(zeroOne,$1a10.completionItem.label);
